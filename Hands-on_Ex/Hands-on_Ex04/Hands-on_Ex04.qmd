...
The chapters below are included in this hands-on exercise, with different install package required.
...
```{r}
pacman::p_load(
  tidyverse, ggdist, ggridges, ggthemes,
  colorspace, ggstatsplot,
  plotly, crosstalk, DT,
  gganimate, FunnelPlotR, knitr, 
  ggforce
)
```

## Chapter 9: Visualising Distribution

### Import data

The practice from Chapter 9 to Chapter 11 uses the same Exam_data file, so one code chunk for data import is enough.
...
To have the area under a ridgeline not filled with a single solid color but rather with colors that vary in some form along the x axis. We can use either [`geom_ridgeline_gradient()`](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html) or [`geom_density_ridges_gradient()`](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html) To achieve the effect. Both geoms work just like `geom_ridgeline()` and `geom_density_ridges()`, except that they allow for varying fill colors.

Important Notes:

- `geom_ridgeline_gradient()` and `geom_density_ridges_gradient()` do not allow for alpha transparency in the fill.
...
Colour the ridgeline plot by quantile, via the calculated `stat(quantile)` aesthetic as shown in the figure below.
...
### Step 1. Plotting a Half Eye graph

Plot a Half-Eye graph by using [`stat_halfeye()`](https://mjskay.github.io/ggdist/reference/stat_halfeye.html) of **ggdist** package.
...
### Step 2. Adding the boxplot with geom_boxplot()

Add the second geometry layer using [`geom_boxplot()`](https://r4va.netlify.app/chap09) of ggplot2. This produces a narrow boxplot. We reduce the width and adjust the opacity.
...
Use the [`coord_flip()`](https://ggplot2.tidyverse.org/reference/coord_flip.html) of ggplot2 package to flip the raincloud chart horizontally to give it the raincloud appearance. At the same time, `theme_economist()` of ggthemes package is used to give the raincloud chart a professional publishing standard look.
...
## Chapter 10: Visual Statistical Analysis
...
## Chapter 11: Visualising Uncertainty
...
## Chapter 8: Funnel Plots for Fair Comparisons
